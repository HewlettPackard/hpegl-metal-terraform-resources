...
output "worker_ips" {
  ...
  value = zipmap(hpegl_metal_host.workers.*.name, hpegl_metal_host.workers.*.connections)
}

output "master_ip" {
  # Output a map of hostame with all the IP addresses assigned on each network.
  value = map(hpegl_metal_host.master.name, hpegl_metal_host.master.connections)
}
...
#   - (proxy configuration if Metal racks are behind a firewall)
#
...
resource "hpegl_metal_host" "master" {
  name         = "master"
  image        = format("%s@%s", data.hpegl_metal_available_images.ubuntu.images[0].flavor, data.hpegl_metal_available_images.ubuntu.images[0].version)
  machine_size = var.msize
  ssh          = var.ssh_keys
  networks     = distinct(concat(["Private", "Public"], [for net in data.hpegl_metal_available_resources.physical.networks : net.name if net.host_use == "Required" && net.location == var.location]))
  ...
  user_data = <<EOF
#cloud-config
# We need mutiple write files with existing Metal svc write-files
...
resource "hpegl_metal_host" "workers" {
  count        = var.workers
...
runcmd:
  - 'export `cat /etc/environment`; curl -sfL https://get.k3s.io | K3S_TOKEN=${random_password.k3s_token.result} K3S_URL=https://${hpegl_metal_host.master.connections.Private}:6443 sh -'
EOF
}
